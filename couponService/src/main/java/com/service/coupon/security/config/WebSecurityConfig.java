...
public class WebSecurityConfig {
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests()
				.antMatchers(HttpMethod.GET, "/getCode/**", "/showGetCoupon", "/getCoupon")
				.hasAnyRole("USER", "ADMIN")
				.antMatchers(HttpMethod.GET, "/showCreateCoupon", "/createCoupon", "/createResponse").hasRole("ADMIN")
				.antMatchers(HttpMethod.POST, "/create", "/saveCoupon").hasRole("ADMIN")
		...
		http.csrf(csrfCustomizer->{
			csrfCustomizer.ignoringAntMatchers("/getCoupon");
		});
